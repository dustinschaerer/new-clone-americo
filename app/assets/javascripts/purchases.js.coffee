...
jQuery ->
  $("#make_same_address").change ->
    if @checked
      ...
      $('input:select[id*=purchase_pay_state]').val($('input:select[id*=purchase_ship_state]').val());
      ...
      $("#purchase_pay_state").prop('enabled', true);
      $('input:text[id*=purchase_pay_state]').val("");
